...
if (-not (Test-Path -LiteralPath $headlessCore -PathType Leaf) -or
    ...
    throw "The reviewed headless core and build manifest are required."
}
$manifestData = Get-Content -LiteralPath $runtimeManifest `
    -Raw -Encoding UTF8 | ConvertFrom-Json
$coreHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $headlessCore).Hash
if ($manifestData.shellMode -ne "headless" -or
    $manifestData.architecture -ne "x64" -or
    $manifestData.qtBuildVersion -ne "6.10.1" -or
    $manifestData.pinnedRuntimeRelease -ne "2.0.0.1736" -or
    $manifestData.coreRuntimeCompatibility -ne
        "uxplay-windows-2.0.0.1736" -or
    -not $manifestData.headlessExecutableSha256 -or
    $coreHash -ne $manifestData.headlessExecutableSha256 -or
    (Get-PeMachine $headlessCore) -ne 0x8664) {
    throw "The core does not match the reviewed headless x64 manifest."
}
